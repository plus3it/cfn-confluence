pipeline {
    ...
    parameters {
        ...
        string(name: 'AcctNum', description: 'Account number for the AWS account')
        string(name: 'AmiSearchString', description: 'Search string to use when searching for AMIs')
        ...
        stage ('Prepare Agent Environment') {
            steps {
                ...
                writeFile file: 'InfraStack.parms.json',
                   ...
                   /
                sh '''#!/bin/bash
                    AmiId=`aws ec2 describe-images --owners ${AcctNum} --filters "Name=name,Values=${AmiSearchString}" --query 'Images[].[Name,CreationDate,ImageId] | sort_by(@,&[1])| [-1][2]'`
                    export AmiId=`echo ${AmiId} | sed -e s'/"//g'`
                    echo "Using ${AmiId}"
                    sed -i "s/MY_AMI_ID/${AmiId}/g" InfraStack.parms.json
                '''
                    }
                }
            }
